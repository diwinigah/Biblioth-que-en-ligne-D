        ...
        <div class="books-grid">
            ...
            function renderBookCard($row) {
                ...
                if(isset($_SESSION["login_user"])) {
                    ...
                    $sql = "SELECT 1 FROM liste_lecture WHERE username = ? AND livre_id = ?";
                    $stmt = mysqli_prepare($db, $sql);
                    mysqli_stmt_bind_param($stmt, "si", $username, $row['id']);
                    mysqli_stmt_execute($stmt);
                    ...
                    <div class="book-footer">
                        ...
                        <?php if(isset($_SESSION["login_user"])): ?>
                            <form method="post" style="display: inline;">
                                ...
                                <?php if($in_list): ?>
                                    <button type="submit" name="remove_list" class="btn btn-xs btn-danger">Retirer</button>
                                <?php else: ?>
                                    <button type="submit" name="add_list" class="btn btn-xs btn-primary">Ajouter</button>
                                <?php endif; ?>
                            </form>
                        <?php else: ?>
            ...
            if(isset($_POST["add_list"])) {
                if(isset($_SESSION["login_user"])) {
                    try {
                        $sql = "INSERT INTO liste_lecture (username, livre_id) VALUES (?, ?)";
                        $stmt = mysqli_prepare($db, $sql);
                        mysqli_stmt_bind_param($stmt, "si", $_SESSION['login_user'], $_POST['book_id']);
                        mysqli_stmt_execute($stmt);
                        echo '<script type="text/javascript">alert("Livre ajouté à votre liste de lecture.");</script>';
                    } catch (mysqli_sql_exception $e) {
                        if ($e->getCode() == 1062) {
                            echo '<script type="text/javascript">alert("Ce livre est déjà dans votre liste de lecture.");</script>';
                        } else {
                            throw $e;
                        }
                    }
            ...
            } elseif(isset($_POST["remove_list"])) {
                if(isset($_SESSION["login_user"])) {
                    $sql = "DELETE FROM liste_lecture WHERE username = ? AND livre_id = ?";
                    $stmt = mysqli_prepare($db, $sql);
                    mysqli_stmt_bind_param($stmt, "si", $_SESSION['login_user'], $_POST['book_id']);
                    if (mysqli_stmt_execute($stmt)) {
                        echo '<script type="text/javascript">alert("Livre retiré de votre liste de lecture.");</script>';
                    } else {
